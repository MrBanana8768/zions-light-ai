...
| Tag | Contents |
|---|---|
| `:v3.1.9.7-cu12` | **Pending review + owner OK** — not yet the deploy target in [runpod.env.template](runpod.env.template). A thin layer FROM the `:v3.1.9.5-cu12` digest: OpenWebUI 0.11.0 → 0.11.4 (the scroll-position fix) plus the `WEBUI_DB_LOCAL=true` local-disk database placement, everything else in `/app/venv` byte-identical outside the five touched packages (CHANGELOG [3.1.9.7], `docs/v3197-pip-diff.txt`). `feature/v3.1.9.7` has `feature/v3.1.9.6` merged into it, so it also carries that branch's operator-script fixes. Migrates `webui.db` on first boot — see RUNPOD_DEPLOY.md before deploying |
| `:v3.1.9.6-cu12` | **Never published as its own image tag.** v3.1.9.6 was an operator-scripts-and-docs-only change (no image bytes differ from `:v3.1.9.5-cu12`), so rather than retag a fourth time it was merged directly into `feature/v3.1.9.7`'s git history and ships as part of that release instead. Do not point a template at this tag — it does not exist on Docker Hub |
| `:v3.1.9.5-cu12` / `:v3.1.9.4-cu12` | **Current release** — the deploy target named in [runpod.env.template](runpod.env.template), until v3.1.9.7 clears review. Same digest, `sha256:c1295894dd58…`, on both tags: v3.1.9.5 changed scripts and docs only, so its tag points at the already-published v3.1.9.4 image instead of a rebuild. CUDA-12 profile (any A40 host) |
...
| `:v3-snapshot` | Frozen V3.3 image (= `:v3.3-tts`), June 2026. Historical: NOT a rollback target from any v3.1.x `/data` (see OPERATIONS.md "Rolling back a bad release") |
...
| [REVIEW_PLAN.md](REVIEW_PLAN.md) | Contributors | How to review the incident-response work — five claim types, five tracks, and the anti-patterns that already cost a day |
